...
export default function Analytics({ humanRatings, allAiRatings, evaluationRuns }: AnalyticsProps) {
  ...
  const allRunAnalytics = useMemo(() => {
    return evaluationRuns.map(run => ({
      run,
      analytics: calculateRunAnalytics(humanRatings, allAiRatings[run.filename] || {})
    }));
  ...
  return (
    <div className="space-y-8">
      ...
      {scatterPlotData.map((runData, runIndex) => {
        const { run, analytics, scatterDataHuman, scatterDataAI, maxX, sortedCandidateIds } = runData;

        return (
          <div key={run.filename} className="space-y-4 border border-stone-200 rounded-lg">
            {/* Run Header */}
            <div className="p-4 border-b border-stone-200 bg-stone-50">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-semibold tracking-tight">
                    {run.model} - {new Date(run.timestamp).toLocaleDateString()}
                  </h2>
                ...
                <div className="text-sm text-muted-foreground">
                  Run {runIndex + 1} of {scatterPlotData.length}
                </div>
              </div>
            </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart margin={{ top: 40, right:20, bottom: 20, left: 0 }}>
                          ...
                          <XAxis
                            ...
                            tickFormatter={(value) => {
                              if (sortByGap) {
                                const candidateId = sortedCandidateIds[value - 1];
                                return candidateId ? `C${candidateId}` : `${value}`;
                              }
                              return `C${value}`;
                            }}
                            ticks={Array.from({ length: maxX }, (_, i) => i + 1)}
                            interval={0}
                          />
                          <YAxis
                            dataKey="y"
